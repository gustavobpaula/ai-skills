...
expected_debug_source=$(CDPATH= cd -P "$repo_root/skills/debug" && pwd)
...
AI_SKILLS_TARGET_DIR="$target_dir" "$installer" spec arch-design feature-implementation requirements-review code-review delivery-check test-strategy arch-review simplify a11y-review debug performance-review >/dev/null
...
[ -L "$target_dir/simplify" ] || fail 'installation did not create the simplify symlink'
...
[ -L "$target_dir/debug" ] || fail 'installation did not create the debug symlink'
[ "$(readlink "$target_dir/debug")" = "$expected_debug_source" ] || fail 'debug symlink is not absolute or points to the wrong source'
...
AI_SKILLS_TARGET_DIR="$target_dir" "$installer" spec arch-design feature-implementation requirements-review code-review delivery-check test-strategy arch-review simplify a11y-review debug performance-review >/dev/null
AI_SKILLS_TARGET_DIR="$target_dir" "$installer" spec arch-design feature-implementation requirements-review code-review delivery-check test-strategy arch-review simplify a11y-review debug performance-review performance-review >/dev/null
...
expect_failure env AI_SKILLS_TARGET_DIR="$partial_target" "$installer" spec arch-design feature-implementation requirements-review code-review delivery-check test-strategy arch-review simplify a11y-review debug performance-review missing-skill
...
[ ! -e "$partial_target/debug" ] || fail 'validation failure caused a partial debug installation'
...
expect_failure env AI_SKILLS_TARGET_DIR="$batch_conflict_target" "$installer" spec arch-design feature-implementation requirements-review code-review delivery-check test-strategy arch-review simplify a11y-review debug performance-review
...
[ ! -e "$batch_conflict_target/debug" ] || fail 'batch conflict caused a partial debug installation'
